...
function ManageProfilesModal() {
    ...
    useClickOutside(ref, () => closeModal("isManageProfilesModalOpen"));
        ...
        >
            <div className="relative w-full h-full flex items-center flex-col justify-center py-16">
                ...
                <form className="flex flex-col gap-6 mb-2 p-10" onSubmit={handleUpdateProfile}>
                    ...
                    <input className="py-2 px-1 rounded-md" id="profile" value={profileName} onChange={e => setProfileName(e.target.value)}></input>
                    ...
                    <div className="flex gap-2">
                        ...
                        <input id="autoplay" type="checkbox" className="w-6 h-6" checked={autoplay} onChange={() => setAutoplay(!autoplay)}></input>
                    </div>
                    <button type="submit" className="py-2.5 bg-[#e50914] rounded-md font-semibold hover:bg-opacity-50 active:scale-105 transition">
                        Save
                    </button>
                </form>
            </div>
