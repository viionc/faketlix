...
function CarouselForwardButton({currentPage, entries, callback}: {currentPage: number; entries: EntryProps[][]; callback: (add: boolean) => void}) {
    return (
        <div
            className={clsx(
                "w-[8%] lg:w-[3%] absolute top-0 right-0 bottom-0 flex justify-center items-center bg-black bg-opacity-50 hover:bg-opacity-70 cursor-pointer z-[1000] rounded-s-md",
